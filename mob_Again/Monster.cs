...
public class Monster : MonoBehaviour, IDamageable
{
    ...
    private void Patrol()
    {
        ...
        if (movingRight)
        {
            ...
            if (newX >= startingX + movementRange)
            {
                movingRight = false;
            }
        }
    ...
    // 기존의 TakeDamage 메서드는 이름을 변경하여 유지
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        {
            Gizmos.DrawLine(
                new Vector3(startingX - movementRange, transform.position.y, 0),
                new Vector3(startingX + movementRange, transform.position.y, 0)
            );
        }
        else
        {
            Gizmos.DrawLine(
                new Vector3(transform.position.x - movementRange, transform.position.y, 0),
                new Vector3(transform.position.x + movementRange, transform.position.y, 0)
            );
        }
    }
